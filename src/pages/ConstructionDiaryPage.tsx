    ...
    <>
      <Container maxWidth="lg" sx={{ py: 5 }}>
        ...
        <Typography variant="h5" color="text.secondary" paragraph>
          Śledź z nami na bieżąco postępy prac na osiedlu [Nazwa Osiedla]. Regularnie publikujemy zdjęcia z placu budowy.
        </Typography>
        ...
        <Paper sx={{ p: { xs: 1, md: 2 }, mt: 4 }}>
          ...
          <ImageList variant="quilted" cols={4} rowHeight={250}>
            {constructionImages.map((item, index) => (
              <ImageListItem key={item.src} cols={index % 5 === 0 ? 2 : 1} rows={index % 5 === 0 ? 2 : 1}>
                <img
                ...
                <ImageListItemBar
                  title={item.title}
                  actionIcon={
                    <IconButton sx={{ color: 'rgba(255, 255, 255, 0.54)' }} onClick={() => handleImageClick(index)}>
                      <ZoomInIcon />
                    </IconButton>
                  }
                />
