...
export function CreditCalculatorPage() {
  ...
  const calculationResults = useMemo(() => {
    ...
    const monthlyInstallment = loanAmount * (monthlyInterestRate * Math.pow(1 + monthlyInterestRate, numberOfPayments)) / 
          ...
          <Grid size={{xs:12,md:6}}>
            <Box sx={{ p: 3, backgroundColor: 'background.default', borderRadius: 2, height: '100%' }}>
              ...
              <Box my={2}>
                ...
                <Typography variant="h4" color="primary">{formatCurrency(calculationResults.downPaymentAmount)}</Typography>
              </Box>
              <Box my={2}>
                <Typography variant="body1">Kwota kredytu:</Typography>
                <Typography variant="h4" color="primary">{formatCurrency(calculationResults.loanAmount)}</Typography>
              </Box>
              <Box mt={3} p={2} bgcolor="primary.main" color="primary.contrastText" borderRadius={1}>
                ...
                <Typography variant="h3" sx={{ fontWeight: 'bold' }}>{formatCurrency(calculationResults.monthlyInstallment)}</Typography>
              </Box>
            </Box>
          </Grid>
        </Grid>
        
        <Paper elevation={0} sx={{ p: 2, mt: 4, backgroundColor: '#fffbe6' }}>
          <Typography variant="body2" color="text.secondary">
            <strong>Zastrzeżenie:</strong> Powyższe obliczenia są symulacją i mają charakter wyłącznie poglądowy. Nie stanowią one oferty handlowej w rozumieniu prawa i nie powinny być podstawą do podejmowania decyzji finansowych. W celu uzyskania wiążącej oferty prosimy o kontakt z doradcą kredytowym.
          </Typography>
        </Paper>
      </Paper>
    </Container>
